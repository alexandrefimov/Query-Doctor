...
def render_optimizer_panel(sql: str) -> str:
    ...
    return (
        ...
        "<div class=\"section-kicker\">Read-only optimization hints for one Impala SELECT/WITH statement.</div>"
        "</div></div>"
        ...
        "<div class=\"info-body\">Only one SELECT/WITH statement is supported. The text is parsed locally, "
        "is not executed, and is not rendered back after submit.</div></details></div>"
        ...
        "<strong>Scope:</strong> local parse · no execution · referenced tables only · allowlisted metadata"
        "</div>"
        "<div class=\"scope-line\" aria-label=\"Optimizer output scope\">"
        "<strong>Output:</strong> referenced tables · metadata status · deterministic findings · candidate optimizations"
        "</div>"
        "</form></section>"
    )
