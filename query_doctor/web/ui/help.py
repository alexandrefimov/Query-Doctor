...
def render_help_content(*, llm_enabled: bool = True, language: str = "en") -> str:
    ...
    action_copy = (
        "<p><strong>Reports and optimizer</strong> contains explicit buttons for LLM Report, Query LLM optimizer, and combined report + optimizer execution. Outputs are rendered only after deterministic validation. If validation rejects generated output, partial content stays untrusted and hidden.</p>"
        if llm_enabled
...
def render_help_content_ru(*, llm_enabled: bool = True) -> str:
    ...
    optimizer_label = "Query LLM optimizer" if llm_enabled else "Query optimizer"
    selected_action_line = (
        "Запускайте <strong>LLM-отчет</strong> или <strong>Query LLM optimizer</strong> только со страницы Details выбранного кейса."
        if llm_enabled
        else "Запускайте <strong>Python-отчет</strong> или <strong>Query optimizer</strong> только со страницы Details выбранного кейса."
    )
    web_scan_boundary = (
        "Web-сканы не запускают LLM-отчеты или optimizer drafts автоматически."
        if llm_enabled
    ...
    action_copy = (
        "<p><strong>Отчеты и оптимизатор</strong> содержит явные кнопки для LLM-отчета, Query LLM optimizer и combined report + optimizer execution. Outputs рендерятся только после deterministic validation; rejected или partial output остается hidden.</p>"
        if llm_enabled
    ...
    report_copy = (
        "<p>Факты анализатора остаются источником истины. LLM отвечает только за формулировки. Язык trusted report выбирается общим полем <code>language</code>; raw LLM output не считается trusted, пока Python validation не примет результат.</p>"
        if llm_enabled
