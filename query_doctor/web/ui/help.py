...
def render_help_page(settings: Any) -> str:
    ...
    return render_page(
        ...
        extra_sections=[render_help_content(llm_enabled=llm_enabled)],
    )
...
def render_help_content(*, llm_enabled: bool = True) -> str:
    content = """
...
""".strip()
    if llm_enabled:
        return content
    return (
        content.replace("Details and LLM actions", "Details and Python-only actions")
        .replace(
            "Run <strong>LLM Report</strong> or <strong>Query LLM optimizer</strong> only from a selected Details page.",
            "Run <strong>Python Report</strong> or <strong>Query optimizer</strong> only from a selected Details page.",
        )
        .replace(
            "Web scans do not auto-run LLM reports or optimizer drafts.",
            "Web scans do not auto-run reports or optimizer jobs.",
        )
        .replace(
            ...
            "It collects and analyzes one query without automatic report or optimizer execution, clears the input after submit, and appends the result to the Known Query ID analysis table.",
        )
        .replace(
            "<p><strong>LLM actions</strong> contains explicit buttons for LLM Report, Query LLM optimizer, and combined report + optimizer execution. Outputs are rendered only after deterministic validation. If validation rejects generated output, partial content stays untrusted and hidden.</p>",
            "<p><strong>Python-only actions</strong> contains explicit buttons for Python Report, Query optimizer, and combined report + optimizer execution. Outputs are rendered only after deterministic validation. If validation rejects generated output, partial content stays untrusted and hidden.</p>",
        )
        .replace(
            "<p>Analyzer facts are the source of truth. The LLM owns wording only. Trusted reports default to English, Russian remains available through the same language-specific prompt, normalizer, and validator boundary, and raw LLM output is never trusted until validation passes.</p>",
            "<p>Analyzer facts are the source of truth. In Python-only mode, reports are generated by deterministic Python without LLM calls. Trusted reports default to English, and raw profile text or SQL stays outside trusted output.</p>",
        )
        .replace(
            "<summary>Details-page Query LLM optimizer</summary>",
            "<summary>Details-page Query optimizer</summary>",
        )
        .replace(
        ...
        .replace(
            ...
            "To avoid trusted-looking output without a selected case. Report generation remains an explicit user action.",
        )
    )
