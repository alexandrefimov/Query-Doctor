...
def validate_draft_sql(
    ...
    for keyword in TOP_LEVEL_SHAPE_KEYWORDS:
        if top_level_keyword_count(source_sql, keyword) != top_level_keyword_count(
            draft_sql, keyword
        ):
            errors.append(f"optimized draft changes top-level {keyword} shape")
    for operator in TOP_LEVEL_SET_OPERATORS:
        if top_level_keyword_count(source_sql, operator) != top_level_keyword_count(
            draft_sql, operator
        ):
    ...
    if cte_name_signature(source_sql) != cte_name_signature(draft_sql):
        if rewrite_recipe and rewrite_recipe.recipe_id == "pass_through_cte_elimination":
            errors.extend(
                validate_pass_through_cte_elimination_rewrite(source_sql, draft_sql, rewrite_recipe)
            )
        else:
            errors.append("optimized draft changes CTE shape")
    elif cte_name_signature(source_sql) and normalized_statement_signature(
        source_sql
    ) != normalized_statement_signature(draft_sql):
        if rewrite_recipe:
    ...
    if top_level_join_condition_signature(source_sql) != top_level_join_condition_signature(
        draft_sql
    ):
...
def validate_recipe_backed_cte_rewrite(
    ...
    if rewrite_recipe.recipe_id == "single_cte_projection_alias_predicate_pushdown":
        return validate_single_cte_projection_alias_predicate_pushdown_rewrite(
    ...
    if (
        not source_union_body
        or not draft_union_body
        or not source_aggregate_body
        or not draft_aggregate_body
    ):
        return ["optimized draft violates rewrite recipe: required CTEs are missing"]
    errors.extend(
        validate_unrelated_cte_bodies_preserved(
            source_ctes, draft_ctes, {rewrite_recipe.source_cte, rewrite_recipe.aggregate_cte}
        )
    )
    ...
    if valid_branch_modes == {"input_rollup"} and normalized_statement_signature(
        source_aggregate_body
    ) != normalized_statement_signature(draft_aggregate_body):
        errors.append(
    ...
    if not identifier_referenced(draft_aggregate_body, rewrite_recipe.source_cte):
        errors.append(
            "optimized draft violates rewrite recipe: downstream aggregate no longer reads the source CTE"
        )
    if keyword_count_any_depth(draft_aggregate_body, "GROUP") == 0:
        errors.append(
            "optimized draft violates rewrite recipe: downstream safety aggregate was removed"
        )
    if not aggregate_projection_names(draft_aggregate_body):
        errors.append(
            "optimized draft violates rewrite recipe: downstream aggregate measures were removed"
        )
    ...
    if not counter_is_subset(
        sql_clause_signature_counter(source_sql, "ON"),
        sql_clause_signature_counter(draft_sql, "ON"),
    ):
        errors.append("optimized draft violates rewrite recipe: source JOIN predicates changed")
    if not counter_is_subset(
        sql_string_literal_counter(source_sql), sql_string_literal_counter(draft_sql)
    ):
...
def validate_pass_through_cte_elimination_rewrite(
    ...
    if draft_names != expected_names:
        errors.append(
            "optimized draft violates rewrite recipe: only the pass-through CTE may be removed"
        )
...
def validate_single_cte_predicate_pushdown_rewrite(source_sql: str, draft_sql: str) -> list[str]:
    ...
    errors.extend(
        validate_cte_predicate_pushdown_segments(source_parsed, source_segments, draft_segments)
    )
    return dedupe_preserve_order(errors)


def validate_single_cte_projection_alias_predicate_pushdown_rewrite(
    ...
    errors.extend(
        validate_cte_segment_shape_preserved_except_where(
            source_cte.body, draft_cte.body, source_cte.name
        )
    )
    errors.extend(
        validate_cte_segment_shape_preserved_except_where(
            source_parsed.final_sql, draft_parsed.final_sql, "final SELECT"
        )
    )
    if sql_predicate_signature_counter(
        source_parsed.final_sql, "WHERE"
    ) != sql_predicate_signature_counter(draft_parsed.final_sql, "WHERE"):
        errors.append(
            "optimized draft violates rewrite recipe: final SELECT WHERE predicates changed"
        )
...
def validate_cte_dag_predicate_pushdown_rewrite(source_sql: str, draft_sql: str) -> list[str]:
    ...
    if source_names != draft_names:
        return ["optimized draft violates rewrite recipe: CTE order changed"]
    if not is_cte_dag_predicate_pushdown_candidate(
        source_sql
    ) or not is_cte_dag_predicate_pushdown_candidate(draft_sql):
...
def validate_single_derived_table_predicate_pushdown_rewrite(
    source_sql: str, draft_sql: str
) -> list[str]:
    errors: list[str] = []
    ...
    if analyze_derived_table_shape(source_sql).predicate_pushdown_status != "candidate":
        errors.append(
            "optimized draft violates rewrite recipe: source derived-table shape is unsupported"
        )
    if analyze_derived_table_shape(draft_sql).predicate_pushdown_status != "candidate":
        errors.append(
            "optimized draft violates rewrite recipe: draft derived-table shape is unsupported"
        )
...
def validate_cte_predicate_pushdown_segments(
    ...
    errors: list[str] = []
    source_predicates = [
        sql_predicate_signature_counter(segment_sql, "WHERE") for _, segment_sql in source_segments
    ]
    downstream_indexes = cte_downstream_segment_indexes(source_parsed)
    for index, ((segment_name, source_segment), (_, draft_segment)) in enumerate(
        zip(source_segments, draft_segments)
    ):
        errors.extend(
            validate_cte_segment_shape_preserved_except_where(
                source_segment, draft_segment, segment_name
            )
        )
    ...
    *,
    ...
    for decision in per_conjunct_pushdown_plan(
        ...
        if decision.copyable and decision.dequalified is not None:
            predicates.update(
                sql_predicate_signature_counter(f"SELECT 1 WHERE {decision.dequalified}", "WHERE")
            )
    return predicates
...
def validate_cte_segment_shape_preserved_except_where(
    ...
    if main_select_has_distinct(source_segment) != main_select_has_distinct(draft_segment):
        errors.append(
            f"optimized draft violates rewrite recipe: {segment_name} changes DISTINCT shape"
        )
    for keyword in TOP_LEVEL_SHAPE_KEYWORDS:
        if top_level_keyword_count(source_segment, keyword) != top_level_keyword_count(
            draft_segment, keyword
        ):
            errors.append(
                f"optimized draft violates rewrite recipe: {segment_name} changes {keyword} shape"
            )
    for operator in TOP_LEVEL_SET_OPERATORS:
        if top_level_keyword_count(source_segment, operator) != top_level_keyword_count(
            draft_segment, operator
        ):
            errors.append(
                f"optimized draft violates rewrite recipe: {segment_name} changes {operator} shape"
            )
    ...
    if top_level_join_condition_signature(source_segment) != top_level_join_condition_signature(
        draft_segment
    ):
        errors.append(
            f"optimized draft violates rewrite recipe: {segment_name} changes JOIN predicates"
        )
    for keyword in ("GROUP", "HAVING", "ORDER", "LIMIT"):
        if clause_signature(source_segment, keyword) != clause_signature(draft_segment, keyword):
            errors.append(
                f"optimized draft violates rewrite recipe: {segment_name} changes {keyword} expression"
            )
    ...
    if source_projection and draft_projection:
        if source_projection.count != draft_projection.count:
            errors.append(
                f"optimized draft violates rewrite recipe: {segment_name} changes projection count"
            )
        elif (
            len(source_projection.output_names) == source_projection.count
            and len(draft_projection.output_names) == draft_projection.count
            ...
            errors.append(
                f"optimized draft violates rewrite recipe: {segment_name} changes projection names"
            )
    ...
    if (
        ...
        errors.append(
            f"optimized draft violates rewrite recipe: {segment_name} changes projection expressions"
        )
    return errors
...
def validate_final_union_distinct_rollup_rewrite(
    ...
    if not identifier_referenced(draft_parsed.final_sql, rewrite_recipe.source_cte):
        errors.append(
            "optimized draft violates rewrite recipe: final aggregate no longer reads the source CTE"
        )
    ...
    errors.extend(
        validate_final_union_distinct_rollup_branch_shape(
            source_union_body, draft_union_body, source_parsed.final_sql
        )
    )
    if not where_predicates_preserved_or_safely_extended_by_union_branch(
    ...
    if not counter_is_subset(
        ...
        errors.append("optimized draft violates rewrite recipe: source string literals changed")
    if not counter_is_subset(
        sql_business_numeric_literal_counter(source_sql),
        sql_business_numeric_literal_counter(draft_sql),
    ):
        errors.append("optimized draft violates rewrite recipe: source numeric literals changed")
    return errors
...
def validate_unrelated_cte_bodies_preserved(
    ...
    for cte_name, source_body in source_ctes.items():
        ...
        if draft_body is None:
            ...
            continue
        if normalized_statement_signature(source_body) != normalized_statement_signature(
            draft_body
        ):
...
def validate_post_union_aggregate_branch_shape(
    ...
    input_rollup_names = post_union_aggregate_input_rollup_names(
        source_union_body, source_aggregate_body
    )
    measure_required_names = tuple(dimensions) + tuple(measures)
    input_required_names = tuple(dimensions) + tuple(input_rollup_names)
    measure_detail_names = post_union_unused_detail_projection_names(
        source_union_body, measure_required_names
    )
    input_detail_names = post_union_unused_detail_projection_names(
        source_union_body, input_required_names
    )
    for index, branch in enumerate(draft_branches, start=1):
        ...
        if expected_projection_counts and len(projection_names) not in expected_projection_counts:
            errors.append(
                f"optimized draft violates rewrite recipe: branch {index} projection shape does not match pushed aggregate"
            )
        missing_dimensions = [name for name in dimensions if name not in projection_name_set]
        if missing_dimensions:
            errors.append(
                f"optimized draft violates rewrite recipe: branch {index} is missing grouped dimensions"
            )
        missing_measures = [name for name in measures if name not in projection_name_set]
        missing_input_rollup_names = [
            name for name in input_rollup_names if name not in projection_name_set
        ]
        if missing_measures and (not input_rollup_names or missing_input_rollup_names):
            errors.append(
                f"optimized draft violates rewrite recipe: branch {index} is missing aggregate measures"
            )
        if input_rollup_names and missing_input_rollup_names and missing_measures:
            errors.append(
                f"optimized draft violates rewrite recipe: branch {index} is missing aggregate input columns"
            )
        extra_detail_names = [
            name
        ...
        if extra_detail_names:
            errors.append(
                f"optimized draft violates rewrite recipe: branch {index} still projects detail-only columns"
            )
        if keyword_count_any_depth(branch, "GROUP") == 0:
            errors.append(
                f"optimized draft violates rewrite recipe: branch {index} is not pre-aggregated"
            )
        if not branch_aggregate_names:
            errors.append(
                f"optimized draft violates rewrite recipe: branch {index} has no aggregate measures"
            )
...
def validate_final_union_distinct_rollup_branch_shape(
    ...
    unused_detail_names = tuple(
        name for name in union_outputs if name and name not in required_name_set
    )
    ...
    for index, branch in enumerate(draft_branches, start=1):
        ...
        if expected_projection_count and len(projection_names) != expected_projection_count:
            errors.append(
                f"optimized draft violates rewrite recipe: branch {index} projection shape does not match pushed distinct rollup"
            )
        missing_required = [name for name in required_names if name not in projection_name_set]
        if index == 1 and missing_required:
            errors.append(
                f"optimized draft violates rewrite recipe: branch {index} is missing final aggregate input columns"
            )
        if (
            index > 1
            and first_branch_projection_names
            and len(projection_names) == len(first_branch_projection_names)
        ):
            misplaced_required = [
                name
            ...
            if misplaced_required:
                errors.append(
                    f"optimized draft violates rewrite recipe: branch {index} projection order does not match the CTE output schema"
                )
        ...
        branch_aggregate_names = set(aggregate_projection_names(branch))
        missing_additive_inputs = [
            name for name in additive_inputs if name not in branch_aggregate_names
        ]
        if missing_additive_inputs:
            errors.append(
                f"optimized draft violates rewrite recipe: branch {index} is missing aggregate measure inputs"
            )
    return dedupe_preserve_order(errors)


def post_union_unused_detail_projection_names(
    source_union_body: str, required_names: Iterable[str]
) -> tuple[str, ...]:
