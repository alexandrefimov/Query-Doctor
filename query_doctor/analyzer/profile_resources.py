...
MIN_MAX_RE = re.compile(
    r"\bmin\s*/\s*max\s*:\s*(?P<min>[^/]+?)\s*/\s*(?P<max>[^,]+)", re.IGNORECASE
)
PERCENTILE_RE = re.compile(
    r"(?P<label>\d+(?:\.\d+)?)th\s+%-ile\s*:\s*(?P<value>[^,]+)", re.IGNORECASE
)
...
def build_profile_resource_facts(text: str) -> dict[str, Any]:
    ...
    bytes_read_values = (
        parse_size_pairs(bytes_read_match.group("value")) if bytes_read_match else []
    )
    user_time_values = (
        parse_duration_pairs(user_time_match.group("value")) if user_time_match else []
    )
    system_time_values = (
