...
def render_top_nav(active: str) -> str:
    ...
    optimizer_class = "nav-link nav-link--active" if active == "optimizer" else "nav-link"
    ...
    return (
        ...
        f"<a class=\"{optimizer_class}\" href=\"/optimizer\">Optimizer</a>"
        f"<a class=\"{help_class}\" href=\"/help\">Справка</a>"
        "</nav>"
    )
...
def render_batch_case_not_found_page(settings: Any, case_id: str) -> str:
    ...
    section = (
        "<section class=\"panel batch-panel\" aria-label=\"Recent scan case not found\">"
        "<div class=\"batch-head\"><div><h1>Recent scan case not found</h1>"
...
def render_batch_case_report_page(settings: Any, case_id: str, case: dict[str, Any], report_text: str) -> str:
    ...
    section = (
        "<section class=\"panel report-header\" aria-label=\"Recent scan case report header\">"
        "<div class=\"breadcrumb\"><a href=\"/\">Recent scan</a><span>/</span>"
        ...
        "<h1>Validated Recent scan case report</h1>"
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  var scanModeHelp = document.getElementById('scan-mode-help');
  var scanModeInputs = document.querySelectorAll('input[name="analysis_depth"]');
  function updateScanModeHelp() {
    if (!scanModeHelp) { return; }
    var selected = document.querySelector('input[name="analysis_depth"]:checked');
    if (!selected) { return; }
    scanModeHelp.textContent = selected.value === 'full'
      ? scanModeHelp.getAttribute('data-full-help')
      : scanModeHelp.getAttribute('data-fast-help');
  }
  scanModeInputs.forEach(function (input) {
    input.addEventListener('change', updateScanModeHelp);
  });
  updateScanModeHelp();
