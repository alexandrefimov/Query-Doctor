...
@media(max-width:980px){.report-shell{grid-template-columns:1fr}.side-panel{position:static}.batch-metrics,.case-summary-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.batch-form-grid{grid-template-columns:repeat(2,minmax(120px,1fr))}.batch-progress-steps{grid-template-columns:repeat(3,minmax(0,1fr))}.optimizer-findings{grid-template-columns:1fr}}@media(max-width:760px){.page{padding:18px 16px 40px}.app-header{align-items:stretch;flex-direction:column}.top-nav{width:fit-content}.run-main-row,.batch-primary-row{grid-template-columns:1fr}.batch-form .run-button{width:100%}.run-secondary-row{align-items:stretch;flex-direction:column}.mode-control,.run-button{width:100%}.segmented{width:100%}.report-title-row{align-items:stretch;flex-direction:column}.batch-metrics,.batch-form-grid,.case-summary-grid{grid-template-columns:1fr}.batch-progress-steps{grid-template-columns:1fr}}
""".strip()
    ...
    *,
    ...
    if job is not None:
        ...
        if job.status == "ok" and getattr(job, "kind", "") == "batch":
            result_html = render_batch_card(settings, query_group=query_group)
        sections.append(render_job_panel(job, result_html_override=result_html))
    if job is None or job.status != "ok":
        batch_card = render_batch_card(settings, query_group=query_group)
        if batch_card:
...
def render_job_panel(job: Any, *, result_html_override: str | None = None) -> str:
    ...
    progress_note = "" if getattr(job, "kind", "") == "batch" else "This usually takes a few seconds to a couple of minutes."
    progress_note_html = f"<p class=\"progress-note\">{progress_note}</p>" if progress_note else ""
    if job.status == "ok":
