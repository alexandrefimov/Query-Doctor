...
| Storage-aware scan diagnostics | P1 | HDFS locality, remote HDFS, S3, ADLS/ABFS, Ozone, and remote-read data cache have different scan semantics. | Derive a safe `storage_context`; do not expose paths, object URIs, credentials, hosts, or treat object-store remote reads as HDFS locality failures. | Future; current storage/HDFS promotion requires mapped scan/storage context and large `TotalBytesRead`, but storage-family semantics remain unknown. |
...
1. Validate the implemented profile evidence-tier gates against sanitized real
   ...
   exchange/data-movement, and storage/HDFS gates are implemented; the next
   proof point is real-case calibration and old-source compatibility, not
   stronger wording.
