...
Русские страницы в `docs/i18n/ru/` - это локализованные сопроводительные
документы для длинных операторских, демо- или обучающих объяснений.
...
Русские сопроводительные документы покрывают текущие английские Markdown-
документы из `docs/` и основной README. Для длинных документов это не всегда
дословный перевод: часть страниц является кратким русским резюме со ссылкой на
канонический английский источник.

Приоритет дальнейшей локализации:

- постепенно расширять summary-страницы до полных переводов там, где документ
  нужен операторам или задает границу доверия;
- сначала поддерживать `safety-contract`, `architecture`, `configuration`,
  `security-model` и `query-optimizer-contract`;
