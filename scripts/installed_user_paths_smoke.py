...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    try:
        prepared = prepare_smoke_work_dir(
            ...
            temp_prefix="query-doctor-installed-user-paths-",
            protected_roots=(ROOT,),
        )
        work_dir = prepared.path
        run_smoke(args, work_dir)
    except SmokeWorkDirError as exc:
        print(f"[installed-user-paths-smoke] FAILED: {exc}", file=sys.stderr)
        if requested_work_dir is not None:
            print(f"[installed-user-paths-smoke] work dir: {requested_work_dir}", file=sys.stderr)
        return 1
    finally:
        if "prepared" in locals() and prepared.cleanup:
            ...
            print(f"[installed-user-paths-smoke] work dir: {prepared.path}")
